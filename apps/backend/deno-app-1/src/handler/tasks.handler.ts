...
export const listTask: RouteHandler<ListTaskRoute> = async (c) => {
    try{
        ...
        if(err) return c.json(internalServerErrorResponse(err),CONSTANTS.STATUS_CODES.INTERNAL_SERVER_ERROR);
    }
};
...
export const createTask:RouteHandler<CreateTask> = async (c) => {
    try{
        ...
        if(err===CONSTANTS.ERROR.INVALID_REQUEST_DATA) return c.json(badRequestResponse(err),CONSTANTS.STATUS_CODES.BAD_REQUEST);
        else return c.json(internalServerErrorResponse(err),CONSTANTS.STATUS_CODES.INTERNAL_SERVER_ERROR);
    }
}
...
if (updatedTask.length === 0) {
  ...
  return c.json(successResponse(updatedTask[0]),CONSTANTS.STATUS_CODES.OK);
    ...
    if (err) return c.json(badRequestResponse, CONSTANTS.STATUS_CODES.BAD_REQUEST);
  }}
